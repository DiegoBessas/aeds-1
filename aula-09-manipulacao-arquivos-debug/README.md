...
> **Observação sobre os caminhos de arquivo:** nos scripts enviados, a leitura do CSV é feita com um caminho absoluto do computador de origem (`open(r"C:\Users\diego\...\AULA 9\arquivo1.csv", "r")`), diferente do gabarito da aula, que usa um caminho relativo (`open("arquivo1.csv")`). Isso significa que os scripts, como estão, só rodam corretamente no computador em que foram escritos. Se quiser, posso ajustar os caminhos para relativos (`open("arquivo1.csv", "r")`) — como o CSV já está na mesma pasta no repositório, o código passaria a funcionar em qualquer máquina; mantive o código como está por enquanto porque o objetivo aqui é documentar o material tal como foi entregue.
...
```bash
cd aula-09-manipulacao-arquivos-debug
python3 exercicio-01-notas-alunos/exercicio_01.py
python3 exercicio-02-busca-aluno/exercicio_02_dicionario_menu.py
```

> Necessário ajustar o caminho do arquivo `arquivo1.csv` dentro do script para o caminho local antes de executar (ver observação acima).
